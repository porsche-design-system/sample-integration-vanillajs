...
export async function getAllPPNDealersForMarket(env, dealerApiUrl, country) {
  return getPPNDealers(env, dealerApiUrl, { latitude: 0, longitude: 0 }, country, { radius: 10000000 });
}
export async function getPPNDealers(env, dealerApiUrl, coordinates, country, options = {}) {
  ...
  try {
    ...
    if (env === Environments.LOCAL) {
      headers.set('Authorization', 'Basic ' + windowService.btoa(`${'DEALER_SEARCH_API_USER'}:${'DEALER_SEARCH_API_PASS'}`));
    }
    const requestParams = {
...
export async function getPPNDealer(env, dealerApiUrl, id, coordinates, country) {
...
export async function getPPNDealerByRegion(id, region) {
...
export async function getPPNDealersByRegion(region) {
