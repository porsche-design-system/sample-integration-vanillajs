...
export function DealerSearchFormItemCard({ dealer, showOverlay, locale }) {
  ...
  return (h("phn-menu-item-card", { tabIndex: 0, text: name, class: `animated ${showOverlay ? 'dealer-card-overlay' : ''} dealer-card`, onClick: () => onDealerCardClick(ppnDealer), analyticsId: ppnDealer.id }, h("div", { class: "phn-flex-row phn-justify-between phn-align-center" }, country === 'JP' ? (h("phn-p-text", { color: "primary", size: "small" }, `〒${ppnDealer.address.postalCode} ${address.city} ${address.street}`)) : (h("phn-p-text", { color: "primary", size: "small" }, `${address.street}, `, h("span", { class: "city" }, ppnDealer.address.postalCode, " ", address.city))), distance && distance.value >= 1 && numberOfDealers > 5 ? (h("phn-p-tag", { color: "notification-info-soft" }, `${Math.round(distance.value)}${distance.unit.toLowerCase()}`)) : null)));
}
