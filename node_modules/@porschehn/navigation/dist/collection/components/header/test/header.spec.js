...
import { firstAccessToken, hashedUserId, mockNewContent, mockPPNDealer, mockPPNDealerSearchResultsWithLocalizedAddress } from "../../../test/test-data";
...
describe('header', () => {
  ...
  describe('data analytics', () => {
    it('should call the global data layer service to configure the global data layer instance and push the load event', async () => {
      ...
      expect((_b = defaultProperties === null || defaultProperties === void 0 ? void 0 : defaultProperties.ids) === null || _b === void 0 ? void 0 : _b.userId).toEqual(hashedUserId);
  ...
  describe('shop state store', () => {
    it('should load shop variables on store with individual props', async () => {
      await newSpecPage({
        components: [Header],
        template: () => h("phn-header", { "wishlist-items-number": "50", "number-of-shopping-items": "10" })
      });
      expect(shopStateStore.state.wishlistItemsNumber).toEqual(50);
      expect(shopStateStore.state.numberOfShoppingItems).toEqual(10);
    });
    it('should watch shop variables and update store with individual props', async () => {
      const page = await newSpecPage({
        components: [Header],
        template: () => h("phn-header", { "wishlist-items-number": "100" })
      });
      await page.waitForChanges();
      page.rootInstance.numberOfShoppingItems = 50;
      expect(shopStateStore.state.wishlistItemsNumber).toEqual(100);
      expect(shopStateStore.state.numberOfShoppingItems).toEqual(50);
    });
