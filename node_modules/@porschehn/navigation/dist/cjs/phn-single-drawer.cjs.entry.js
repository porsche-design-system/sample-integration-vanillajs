...
function DrawerContent({ routingKey, breakpoint, locale, receivedContent }) {
  ...
  return (index.h(index.Fragment, null,
    ...
    index.h("div", { class: "drawer-content" },
      index.h("phn-drawer-header", { showBackButton: routingManager.showBackButton(routingKey), backButtonAnalyticsId: menu.id, breakpoint: breakpoint, route: routingKey, receivedContent: receivedContent, locale: locale }),
...
function registerSingleDrawerAnimationSequences() {
  function dispatchDrawerStatusEvent(status) {
    helper.dispatchEvent('phn-single-drawer', 'phnDrawerStatus', {
      bubbles: true,
      composed: true,
      detail: { status }
    });
  ...
  registerOpenSingleDrawerAnimation({
    onAnimationStarts: () => {
      dispatchDrawerStatusEvent(helper.DrawerStatusValues.startOpening);
    },
    onAnimationEnds: () => {
      onTransitionEnd();
      dispatchDrawerStatusEvent(helper.DrawerStatusValues.finishOpening);
    }
  });
  registerCloseSingleDrawerAnimation({
    onAnimationStarts: () => {
      dispatchDrawerStatusEvent(helper.DrawerStatusValues.startClosing);
    },
    onAnimationEnds: () => {
      dispatchDrawerStatusEvent(helper.DrawerStatusValues.finishClosing);
      routingManager.syncAnimation();
      if (navContentStore.navStateStore.state.usingKeyboard) {
        setTimeout(() => {
          helper.focusOnElement(helper.getHTMLElementFromRoot('phn-burger-button phn-p-button-pure'));
        }, 100);
      }
    }
  });
...
function onTransitionEnd() {
  const focussedElementId = routerStore.routerStore.state.animatingPointer > routerStore.routerStore.state.routingKeyPointer
    ? routerStore.routerStore.state.routingKeyHistory[routerStore.routerStore.state.animatingPointer]
    : '';
  routingManager.syncAnimation();
  routingManager.pop();
  setTimeout(() => {
    const singleDrawer = helper.getHTMLElementFromRoot('.drawer');
...
function sendDataLayerEvent() {
  ...
  globalDataLayerService.globalDataLayerService.pushDataLayerEvent(navContentStore.EventActions.DRAWER_LOAD, {
    componentDisplay: {
      displayElementType: analyticsManager.analyticsManager.calculateDisplayElementType(routingManager.getCurrentRoute(), navContentStore.ElementTypes.singleDrawer),
...
function focusOn(element, focussedElementId) {
  if (!element)
    return;
  const autoFocusElement = helper.getHTMLElement(element.querySelector('.autofocus'));
  ...
  if (navContentStore.navStateStore.state.usingKeyboard) {
    ...
    if (helper.isString(focussedElementId)) {
      focussableElement = helper.getHTMLElement(element.querySelector(`.active [data-id="${focussedElementId}"]`));
    }
    if (focussableElement === null) {
      focussableElement = helper.getHTMLElement(element.querySelector(`.active ${helper.FOCUSABLE_ELEMENTS_SELECTOR}`));
    }
    if (focussableElement !== null) {
      helper.focusOnElement(focussableElement);
    }
  }
  else {
    const firstFocus = helper.getHTMLElement(element.querySelector('.first-focus'));
    if (firstFocus !== null) {
...
const SingleDrawer = class {
  ...
  handleKeyDown(e) {
    ...
    const targetElement = helper.getHTMLElement(e.target);
    if (targetElement === null) {
      return;
    }
    let focusableDrawerElements = helper$1.getAllFocusableElements(this.el.querySelector('.active'));
    if (focusableDrawerElements.length === 0) {
      return;
    }
    if (routingManager.getCurrentRoute().startsWith(routingKey.RoutingKeys.DEALER_SEARCH)) {
    ...
    if (targetElement === firstFocusableDrawerElement && e.shiftKey) {
      ...
      if (lastFocusableDrawerElement !== null) {
        helper.focusOnElement(lastFocusableDrawerElement);
      }
      return;
    }
    if (targetElement === lastFocusableDrawerElement && !e.shiftKey) {
      ...
      if (firstFocusableDrawerElement !== null) {
        helper.focusOnElement(firstFocusableDrawerElement);
      }
      return;
    }
  }
  ...
  disconnectedCallback() {
    ...
    routerStore.routerStore.reset();
  }
  render() {
    const { nextRoute, currentRoute } = routingManager.handleAnimationRouting();
    return (index.h(index.Host, null, helper.isString(currentRoute) ? (index.h("div", { class: "drawer active", key: currentRoute }, index.h("span", { class: "first-focus", tabIndex: -1, "aria-hidden": "true" }), index.h(DrawerContent, { routingKey: currentRoute, breakpoint: this.breakpoint, locale: this.locale, receivedContent: this.receivedContent }))) : null, helper.isString(nextRoute) ? (index.h("div", { class: `drawer animating`, style: routingManager.isAnimatingForward() ? { left: '100%' } : {}, key: nextRoute }, index.h(DrawerContent, { routingKey: nextRoute, breakpoint: this.breakpoint, locale: this.locale, receivedContent: this.receivedContent }))) : null));
  }
