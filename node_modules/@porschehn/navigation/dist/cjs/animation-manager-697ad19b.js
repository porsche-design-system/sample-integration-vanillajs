...
function initialize() {
  helper.getRootElement().addEventListener('transitionend', animationEndListener);
  helper.getRootElement().addEventListener('animationend', animationEndListener);
}
...
function animationEndListener(event) {
  const target = helper.getHTMLElement(event.target);
  if (target === null) {
...
async function executeAnimation({ onStart, onEnd, animation, elementSelector }) {
  ...
  const animatedElements = Array.from(helper.getRootElement().querySelectorAll(elementSelector));
...
//# sourceMappingURL=animation-manager-697ad19b.js.map