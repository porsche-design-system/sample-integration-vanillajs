...
const SideDrawer = class {
  constructor(hostRef) {
    ...
    this.sideDrawerItemClick = index.createEvent(this, "sideDrawerItemClick", 7);
  ...
  render() {
    ...
    const sideDrawerItems = menuItemsService.getSideDrawerItems(navContentStore.navContentStore.state, abTestingConstructorService.dealerSearchResultsStore.state, this.locale, loggedInState, unreadMessagesCount, this.activeRoutingKey, userConsent);
    return (index.h("nav", null, sideDrawerItems.map((_a) => {
      var { routingKeys } = _a, sideDrawerItem = __rest(_a, ["routingKeys"]);
      const isActive = routingKeys.includes(this.activeRoutingKey);
      return (index.h(index.Fragment, null, index.h("phn-side-drawer-item", Object.assign({ key: sideDrawerItem.elementId, onClick: () => {
          if (!naviError.isString(sideDrawerItem.link) && !isActive) {
            this.sideDrawerItemClick.emit(routingKeys);
          }
        }, isActive: isActive, class: naviError.isModelName(sideDrawerItem.text) ? 'models' : '' }, sideDrawerItem)), isActive ? index.h("div", { class: `item-highlight` }) : null));
    })));
  }
};
SideDrawer.style = sideDrawerCss;

exports.phn_side_drawer = SideDrawer;

//# sourceMappingURL=phn-side-drawer.cjs.entry.js.map