...
const contentService = require('./content-service-cd957c0f.js');
...
require('./shop-helper-d333e4b0.js');
...
const CountryRecommender = class {
  constructor(hostRef) {
    ...
    this.bannerClosed = index.createEvent(this, "bannerClosed", 7);
  ...
  async setFlagURL() {
    const footerAssetsUrl = helper.getConfig(navContentStore.navStateStore.state.env).FOOTER_ASSETS_URL;
    this.flagURL = await contentService.getFlagURL(footerAssetsUrl, this.locale);
  }
  closeCountryRecommender() {
    globalDataLayerService.globalDataLayerService.pushDataLayerEvent(navContentStore.EventActions.COUNTRY_RECOMMENDER_CLOSE, {
      componentClick: {
  ...
  componentDidLoad() {
    globalDataLayerService.globalDataLayerService.pushDataLayerEvent(navContentStore.EventActions.COUNTRY_RECOMMENDER_LOAD, {
      componentClick: {
  ...
  handleAnimationEnd(e) {
    ...
    if (e.animationName === 'slideUp') {
      (_a = helper.getHTMLElement(e.target)) === null || _a === void 0 ? void 0 : _a.classList.remove('hiding');
      this.bannerClosed.emit();
    }
    else if (e.animationName === 'slideDown') {
      (_b = helper.getHTMLElement(e.target)) === null || _b === void 0 ? void 0 : _b.classList.remove('showing');
    }
  }
  sendDataLayerEvent() {
    globalDataLayerService.globalDataLayerService.pushDataLayerEvent(navContentStore.EventActions.COUNTRY_RECOMMENDER_CHANGE, {
      componentClick: {
        clickElementId: this.content.change.id,
        clickElementName: this.content.change.text,
        clickElementType: navContentStore.ClickElementTypes.Link,
        targetUrl: helper.getConfig(navContentStore.navStateStore.state.env).COUNTRY_SELECTOR_URL
      }
    });
  }
  render() {
    const { statement, region, language, change } = this.content;
    return (index.h(index.Host, { class: "showing" }, index.h("div", { class: "country-recommender-container" }, index.h("div", { class: "country-recommender" }, index.h("div", { class: "country-recommender-icons-tray" }, index.h("div", { class: "left-icons" }, helper.isString(statement) ? (index.h("phn-p-text", { theme: "dark", size: "inherit", class: "statement" }, statement)) : /* istanbul ignore next */
      null, index.h("div", { class: "selector" }, index.h("div", { class: "phn-flex-row phn-flex-wrap" }, index.h("div", { class: "flag-region" }, helper.isString(this.flagURL) ? (index.h("phn-p-icon", { source: this.flagURL, class: "flag", "aria-hidden": "true", size: "inherit", color: "inherit" })) : /* istanbul ignore next */
      null, helper.isString(region) && helper.isString(language) ? (index.h("phn-p-text", { theme: "dark", class: "language", size: "inherit" }, region, " / ", language)) : /* istanbul ignore next */
      null), index.h("phn-p-link-pure", { theme: "dark", size: "inherit", class: "change", onClick: () => this.sendDataLayerEvent(), aria: { 'aria-label': 'Change Region or Language' } }, index.h("a", { href: helper.getConfig(navContentStore.navStateStore.state.env).COUNTRY_SELECTOR_URL }, change.text))))), index.h("div", { class: "right-icons" }, index.h("phn-p-button-pure", { theme: "dark", icon: "close", onClick: () => this.closeCountryRecommender(), class: "close-button", "hide-label": "true" }, "Close dialog")))))));
  }
...
exports.phn_country_recommender = CountryRecommender;

//# sourceMappingURL=phn-country-recommender.cjs.entry.js.map