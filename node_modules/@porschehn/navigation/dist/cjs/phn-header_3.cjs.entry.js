...
const routerStore = require('./router-store-1a4e63e5.js');
...
const doubleDrawerAnimationSequences = require('./double-drawer-animation-sequences-8a1a8761.js');
...
async function getUserInformation(accessToken, locale) {
  ...
  try {
    const response = await fetch(helper.getString(`${config === null || config === void 0 ? void 0 : config.BFF_ENDPOINT}/user-information?locale=${locale}`), init);
    if (!response.ok) {
    ...
    throw new navContentStore.NaviError(`Get User information request failed in Frontend with error: ${error}`, navContentStore.NaviErrorTypes.GENERAL);
  }
}
...
async function getUserIdFromUserData(userData) {
  return await featureToggles.windowService.hashString(helper.getString(userData.porscheId));
}
const userDataCache = new Map();
async function getUserData(accessToken, locale, config) {
  const cacheKey = helper.constructCacheKey(accessToken, locale, config.PROFILE_API_URL);
  const cachedUserData = userDataCache.get(cacheKey);
  if (cachedUserData !== undefined) {
    return cachedUserData;
  }
  let userProfileResponse;
  const { language, country } = navContentStore.splitLocale(locale);
  const init = {
    method: 'GET',
    headers: {
      'Accept-Language': `${language}_${country}`,
      'x-trace-id': v4(),
      apikey: helper.getString(config.CLIENT_ID),
      Authorization: `Bearer ${accessToken}`,
      Accept: 'application/json'
    }
  };
  const query = country !== navContentStore.RegionLocales.INTERNATIONAL ? `?country=${country}` : '';
  try {
    userProfileResponse = await fetch(`${helper.getString(config.PROFILE_API_URL)}${query}`, init);
  }
  catch (err) {
  ...
  try {
    ...
    throw new navContentStore.NaviError(`Could not extract json from user profile response: ${err}`, navContentStore.NaviErrorTypes.GENERAL);
  }
...
async function initializeUserData(accessToken, locale, config) {
  ...
  finderStateStore.finderStateStore.state.savedSearchesCount = savedSearches;
  finderStateStore.finderStateStore.state.savedVehiclesCount = savedVehicles;
  navContentStore.navStateStore.state.unreadMessagesCount = unreadMessages;
}
...
const Header = class {
  ...
  watchShopVariables(newValue, _, propName) {
    shopHelper.shopStateStore.set(propName, newValue);
  }
  ...
  async setDealerSearchResultsStore() {
    const { savedDealerCookie, ppnDealers } = abTestingConstructorService.dealerSearchResultsStore.state;
    ...
    try {
      ...
      abTestingConstructorService.dealerSearchResultsStore.state.ppnDealers[savedDealerCookie.id] = ppnDealer;
    }
    catch (err) {
...
const Level0 = class {
  ...
  handleClickDrawerHeaderClose() {
    if (singleDrawerAnimationSequences.isPlayingSingleDrawerAnimation() || doubleDrawerAnimationSequences.isPlayingDoubleDrawerAnimation()) {
      return;
    }
  ...
  handleDrawerChanged({ detail: isNavigatingBackwards }) {
    ...
    const { ppnDealer: savedDealer } = abTestingConstructorService.dealerSearchResultsStore.state.ppnDealers[dealerId] || {};
  ...
  updateRouterStore() {
    routerStore.routerStore.state.routingKeyHistory = this.routingKeyHistory;
    routerStore.routerStore.state.routingKeyPointer = this.routingKeyPointer;
  }
  handleDrawerClosed() {
    this.drawerClosed = true;
  ...
  getLogoSize() {
    if (featureToggles.featureToggles.isFeatureEnabled(featureToggles.FEATURE_PORSCHE_75_YEARS_LOGO, featureToggles.windowService.location.search)) {
      return 'medium';
    }
  ...
  render() {
    ...
    return (index.h(index.Host, { class: `${helper.constructThemeClass(this.mode, this.theme)}-theme themed-background` }, index.h("header", { class: this.limitedWidth ? 'limited-width' : '' }, renderBackdrop ? (index.h("phn-backdrop", { activeRoutingKey: this.routingKeyHistory[this.routingKeyPointer], style: this.isLoading ? { display: 'none' } : {} })) : null, index.h("div", { class: "level-0-content" }, !this.displayLogoOnly ? (index.h("div", { class: "level-0-content-icons" }, index.h("phn-level-0-icons", { locale: this.locale, theme: this.theme, mode: this.mode, app: this.app, breakpoint: this.breakpoint, isLoading: this.isLoading, isNavigationLoaded: this.isNavigationLoaded }, index.h("div", { class: "level-0-content-logo" }, index.h("phn-logo", { app: this.app, locale: this.locale, theme: this.theme, mode: this.mode, size: this.getLogoSize(), breakpoint: this.breakpoint, style: this.isNavigationLoaded ? {} : { display: 'none' } }), !this.isNavigationLoaded ? (index.h("div", { class: "crest-skeleton skeleton-container" }, this.breakpoint > helper.breakpoint.xs ? index.h("span", { class: "label-bone" }) : index.h("span", { class: "crest-bone" }))) : null)), shopHelper.isShopAvailable(this.locale, this.app) ? (index.h("contextual-drawer", null, index.h("div", { slot: "contextual_drawer_header" }, index.h("slot", { name: "contextual_drawer_header" })))) : null)) : (index.h("div", { class: "level-0-content-logo" }, index.h("phn-logo", { app: this.app, locale: this.locale, theme: this.theme, mode: this.mode, size: this.getLogoSize(), breakpoint: this.breakpoint, style: this.isNavigationLoaded ? {} : { display: 'none' } }), !this.isNavigationLoaded ? (index.h("div", { class: "crest-skeleton skeleton-container" }, this.breakpoint > helper.breakpoint.xs ? index.h("span", { class: "label-bone" }) : index.h("span", { class: "crest-bone" }))) : null))), !this.drawerClosed ? (helper.isDesktop(this.breakpoint) ? (index.h("phn-double-drawer", { class: "drawer", style: this.isLoading ? { display: 'none' } : {}, routingKeyHistory: this.routingKeyHistory, routingKeyPointer: this.routingKeyPointer, breakpoint: this.breakpoint, initialUsingKeyboard: this.usingKeyboard, locale: this.locale, receivedContent: this.receivedContent })) : (index.h("phn-single-drawer", { class: "drawer", style: this.isLoading ? { display: 'none' } : {}, routingKeyHistory: this.routingKeyHistory, routingKeyPointer: this.routingKeyPointer, breakpoint: this.breakpoint, usingKeyboard: this.usingKeyboard, locale: this.locale, receivedContent: this.receivedContent }))) : null)));
  }
...
const Wrapper = class {
  ...
  componentWillLoad() {
    ...
    animationManager.animationManager.initialize(this.el);
  }
  render() {
