...
const dealerSearchHelper = require('./dealer-search-helper-5097d36e.js');
const menuItemsService = require('./menu-items-service-e380c308.js');
const abTestingConstructorService = require('./ab-testing-constructor-service-c7ed262c.js');
const shopHelper = require('./shop-helper-c61c217b.js');
const routingKey = require('./routing-key-39c28377.js');
const finderStateStore = require('./finder-state-store-cdb73122.js');
...
const CarBodyTypeDrawer = class {
  ...
  getImageSize(currentBreakpoint) {
    switch (currentBreakpoint) {
      case naviError.breakpoint.xxl:
        return 1920;
      case naviError.breakpoint.xl:
      case naviError.breakpoint.l:
        return 1360;
      case naviError.breakpoint.m:
        return 1024;
      case naviError.breakpoint.s:
        return 760;
      case naviError.breakpoint.xs:
      case naviError.breakpoint.base:
      default:
        return 320;
    }
  }
  getImageSource(image) {
    const assetPath = index.getAssetPath(`${naviError.getConfig(navContentStore.navStateStore.state.env).ASSETS_URL}/model-assets`);
...
const Level1 = class {
  ...
  showRightIconRight(iconRight) {
    ...
    const isOneUiMobileShop = shopHelper.isShopAvailable(this.locale, app) && !naviError.isDesktop(this.breakpoint);
  ...
  render() {
    ...
    return (index.h("nav", null, level1Items.map((_a) => {
      ...
      return (index.h("phn-menu-item", Object.assign({ key: elementId, elementId: elementId, onClick: () => {
        ...
        }, iconRight: iconRight !== undefined ? this.showRightIconRight(iconRight) : 'arrow-head-right', variant: naviError.MenuItemVariant.Level1 }, level1Item), showIconModifier ? (index.h("span", { slot: "icon-modifier", "aria-hidden": "true", class: "myporsche-unread-messages-indicator", "data-test-id": "unread-messages-indicator" })) : null));
    })));
  }
};
...
const Menu = class {
  ...
  showMenu(childItem) {
    if (!naviError.isString(childItem.link)) {
  ...
  render() {
    ...
    const menuItems = displayedItem.children.map((childItem) => (index.h("phn-menu-item", { key: childItem.id, class: `${this.showSkeleton ? 'skeleton' : ''}`, elementId: routingKey.constructRoutingKey(this.routingKey, childItem.id), iconRight: childItem.children.length > 0 && !this.showSkeleton ? 'arrow-head-right' : '', analyticsId: childItem.id, text: childItem.text, link: childItem.link, datalayerEventAction: naviError.isString(childItem.link) ? navContentStore.EventActions.NAVIGATION_LINK_CLICK : navContentStore.EventActions.NAVIGATION_CLICK, onClick: () => this.showMenu(childItem), role: "listitem" })));
...
function Links({ content, onLogoutClick, showSkeleton }) {
  ...
  const isLoggedIn = naviError.IsLoggedIn(loggedInState);
...
function ShopSection({ activeRoutingKey, breakpoint, receivedContent, loginEvent }) {
  const showHeadline = !naviError.isDesktop(breakpoint);
  ...
  const wishListAuthStatusList = naviError.IsLoggedIn(navContentStore.navStateStore.state.loggedInState)
    ? ['loggedIn', null]
    : ['loggedOut', null];
  ...
  return (index.h(index.Fragment, null,
    ...
    activeRoutingKey === routingKey.RoutingKeys.SHOP ? (index.h("div", { class: "wishlist-container", role: "list" }, wishListItems.map((childItem) => {
      return (index.h("phn-menu-item", { key: childItem.id, "data-id": childItem.id, size: 'x-small', class: !(naviError.isString(childItem.iconUrl) || naviError.isString(childItem.icon)) ? 'no-icon' : '', iconLeft: (naviError.isString(childItem.id) && shopHelper.getIconNameByMenuItemId(childItem.id)) ||
          childItem.icon ||
          childItem.iconUrl, elementId: routingKey.constructRoutingKey(activeRoutingKey, childItem.id), analyticsId: childItem.id, text: replaceVariables(childItem.text, shopHelper.getAvailableShopVariables()), link: childItem.link, datalayerEventAction: naviError.isString(childItem.link) ? navContentStore.EventActions.NAVIGATION_LINK_CLICK : navContentStore.EventActions.NAVIGATION_CLICK, onClick: () => {
...
const Router = class {
  ...
  render() {
    ...
    const showHeadline = !naviError.isDesktop(this.breakpoint);
    switch (topLevelRoutingKey) {
      case routingKey.RoutingKeys.DEALER_SEARCH: {
        ...
        if (naviError.isString(dealerId)) {
