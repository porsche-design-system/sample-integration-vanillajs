...
require('./global-data-layer-service-a697b177.js');
require('./analytics-manager-c01a9024.js');
...
function MenuItemText({ text, size, subtext, fontWeight }) {
  return (index.h("div", { class: "skeleton-item" },
    index.h("phn-p-text", { size: size, color: "inherit", tag: "div", weight: fontWeight }, text),
    naviError.isString(subtext) ? (index.h("div", { class: "text-limit" },
      index.h("phn-p-text", { class: 'subtext', color: 'inherit', weight: "regular", size: "x-small", tag: "div", ellipsis: "true", "data-test-id": "subtext" }, subtext))) : null));
}

function Icon({ isLoading, icon }) {
  if (isLoading) {
    return (index.h("div", { class: "icon-container" },
      index.h("phn-p-spinner", { color: "inherit", size: "inherit" })));
  }
  if (naviError.isString(icon)) {
    return (index.h("div", { class: "icon-container skeleton-item" },
      index.h("phn-p-icon", { color: "inherit", size: "inherit", source: icon }),
      index.h("slot", { name: "icon-modifier" })));
  }
  return null;
}
...
const MenuItem = class {
  ...
  render() {
    const body = (index.h("div", { class: "phn-flex-row phn-align-center" }, this.existsSlot('media') ? (index.h("div", { class: "media-container skeleton-item" }, index.h("slot", { name: "media" }))) : null, index.h(Icon, { isLoading: this.isLoading, icon: this.iconLeft }), !this.isModels && naviError.isString(this.text) ? (index.h(MenuItemText, { size: this.size, text: this.text, subtext: this.subtext, fontWeight: this.fontWeight })) : undefined, this.existsSlot('suffix') ? (index.h("div", null, index.h("slot", { name: "suffix" }))) : null));
    if (naviError.isString(this.link)) {
      return (index.h(index.Host, { class: this.variant }, index.h("phn-p-link-pure", { class: `${naviError.isString(this.iconLeft) ? 'has-icon' : ''}`, icon: naviError.isString(this.iconRight) ? this.iconRight : 'none', "align-label": "left", stretch: naviError.isString(this.iconRight) ? 'true' : 'false', "data-id": this.elementId, target: this.target, onClick: () => {
          this.sendDataLayerEvent(navContentStore.ClickElementTypes.Link, this.elementId);
        } }, index.h("a", { href: this.link }, body))));
    }
    return (index.h(index.Host, { class: this.variant }, index.h("phn-p-button-pure", { class: `${naviError.isString(this.iconLeft) ? 'has-icon' : ''}`, size: "inherit", icon: naviError.isString(this.iconRight) ? this.iconRight : 'none', "align-label": "left", stretch: naviError.isString(this.iconRight) ? 'true' : 'false', "data-id": this.elementId, onClick: () => {
...
exports.phn_menu_item = MenuItem;

//# sourceMappingURL=phn-menu-item.cjs.entry.js.map