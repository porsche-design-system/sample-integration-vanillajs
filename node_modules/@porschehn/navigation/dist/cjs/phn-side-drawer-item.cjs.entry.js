...
require('./routing-key-7ce95adb.js');
require('./router-store-fdfbb3ef.js');
require('./menu-items-service-de2bb23d.js');
...
function SideDrawerItemText({ isActive, text, subtext, renderSignatures }) {
  return (index.h("div", { class: "text-container" },
    renderSignatures && helper.isModelName(text) ? (index.h("phn-p-model-signature", { model: text.toLowerCase(), size: "inherit", class: "model-signature" })) : (index.h("phn-p-text", { size: "inherit", weight: isActive ? 'semibold' : 'regular', color: "inherit", tag: "div" }, text)),
    helper.isString(subtext) && !helper.isModelFuelTypeGasoline(text) ? (index.h("div", { class: "text-limit" },
      index.h("phn-p-text", { color: "inherit", class: "subtext", weight: "regular", size: "x-small", tag: "div", ellipsis: "true", "data-test-id": "subtext" }, subtext))) : null));
}
...
const SideDrawerItem = class {
  ...
  sendDataLayerEvent(clickElementType, elementId) {
    globalDataLayerService.globalDataLayerService.pushDataLayerEvent(this.datalayerEventAction, Object.assign(Object.assign(Object.assign({ componentClick: Object.assign({ clickElementId: this.analyticsId, clickElementName: this.text, targetElement: analyticsManager.analyticsManager.getModelTargetElement(elementId, this.text), clickElementType }, (clickElementType === navContentStore.ClickElementTypes.Link ? { targetUrl: this.link } : {})) }, this.customDatalayerProperties), { componentDisplay: {
        displayElementId: analyticsManager.analyticsManager.calculateDisplayElementId(),
        displayElementType: analyticsManager.analyticsManager.calculateDisplayElementType(elementId, navContentStore.ElementTypes.sideDrawerItem, true),
        displayElementName: this.text
      } }), (this.datalayerEventAction === navContentStore.EventActions.MODEL_RANGE_CLICK
      ? {
        car: {
  ...
  render() {
    var _a;
    const body = (index.h("div", { class: "body-container phn-flex-row phn-align-center" }, index.h(SideDrawerItemText, { isActive: false, text: this.text, subtext: this.subtext, renderSignatures: this.renderSignatures }), this.existsSlot('suffix') ? (index.h("div", null, index.h("slot", { name: "suffix" }))) : null));
    if (helper.isString(this.link)) {
      const externalShop = ((_a = navContentStore.navContentStore.state.externalShop) === null || _a === void 0 ? void 0 : _a.link) === this.link;
      return (index.h("phn-p-link-pure", { icon: helper.isString(this.iconRight) ? this.iconRight : 'none', "align-label": "left", stretch: "true", "data-id": this.elementId, onClick: () => {
          this.sendDataLayerEvent(navContentStore.ClickElementTypes.Link, this.elementId);
        } }, index.h("a", { href: this.link, target: externalShop ? '_blank' : '_self' }, body)));
    }
    return (index.h(index.Host, { class: `${this.isActive ? 'active' : ''}` }, index.h("phn-p-button-pure", { active: this.isActive, class: helper.isModelName(this.text) ? 'models' : '', size: "inherit", stretch: "true", "data-id": this.elementId, onClick: () => {
...
SideDrawerItem.style = sideDrawerItemCss;

exports.phn_side_drawer_item = SideDrawerItem;

//# sourceMappingURL=phn-side-drawer-item.cjs.entry.js.map