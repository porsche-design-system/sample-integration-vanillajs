...
async function fetchContent(locale, app) {
  var _a;
  const config = naviError.getConfig(featureToggles.featureToggles.env);
  const isShopEnabled = featureToggles.featureToggles.isShopToggleActive() && app === naviError.Application.shop;
...
async function fetchNaviContent(contentUrl, locale) {
  ...
  const cacheKey = naviError.constructCacheKey(contentUrl, locale);
  ...
  if (data.status === 404) {
    throw new naviError.NaviError(`No content for locale ${locale} found`, naviError.NaviErrorTypes.NO_RESULTS);
  }
  if (!data.ok) {
    throw new naviError.NaviError(`Could not get content for locale ${locale}`, naviError.NaviErrorTypes.GENERAL);
  }
...
async function fetchShopContent(contentUrl, locale) {
  ...
  if (data.status === 404) {
    throw new naviError.NaviError(`No shop content for locale ${locale} found`, naviError.NaviErrorTypes.NO_RESULTS);
  }
  if (!data.ok) {
    throw new naviError.NaviError(`Could not get shop content for locale ${locale}`, naviError.NaviErrorTypes.GENERAL);
  }
  return data.json();
}
...
//# sourceMappingURL=content-service-45add4c7.js.map