...
async function getPPNDealers(env, dealerApiUrl, coordinates, country, options = {}) {
  ...
  try {
    ...
    if (env === navContentStore.Environments.LOCAL) {
      headers.set('Authorization', 'Basic ' + windowService.windowService.btoa(`${'undefined'}:${'undefined'}`));
    }
    const requestParams = {
      coordinates: `${coordinates.latitude},${coordinates.longitude}`,
      radius
    };
    response = await fetch(`${dealerApiUrl}/${country}/dealers?${naviError.getQueryString(requestParams)}`, {
      method: 'GET',
      headers
    });
  }
  catch (err) {
    throw new naviError.NaviError('Unable to fetch dealers', naviError.NaviErrorTypes.GENERAL);
  }
  if (response.status === 400) {
    throw new naviError.NaviError('dealerService.getPPNDealers failed with invalid coordinates', naviError.NaviErrorTypes.GENERAL);
  }
  if (response.status === 404) {
    throw new naviError.NaviError('dealerService.getPPNDealers failed with invalid country code and therefore responded with no results', naviError.NaviErrorTypes.NO_RESULTS);
  }
  if (response.status !== 200) {
    throw new naviError.NaviError('dealerService.getPPNDealers failed with invalid response', naviError.NaviErrorTypes.GENERAL);
  }
  try {
    ppnDealerSearchResults = await response.json();
  }
  catch (err) {
    throw new naviError.NaviError('dealerService.getPPNDealers could not parse response into json format', naviError.NaviErrorTypes.GENERAL);
  }
...
async function getPPNDealer(env, dealerApiUrl, id, coordinates, country) {
  ...
  try {
    ...
    throw new naviError.NaviError('Unable to fetch a dealer', naviError.NaviErrorTypes.GENERAL);
  }
  if (ppnDealer === undefined) {
    throw new naviError.NaviError(`Requested dealer ${id} could not be found`, naviError.NaviErrorTypes.NO_RESULTS);
  }
  return ppnDealer;
}
...
async function getPPNDealerByRegion(id, region) {
  ...
  if (dealerResult === undefined) {
    throw new naviError.NaviError(`Requested dealer could not be found`, naviError.NaviErrorTypes.NO_RESULTS);
  }
  return dealerResult;
}
async function getPPNDealersByRegion(region) {
  ...
  if (dealersByRegionMap.has(region)) {
    ...
    const { BFF_ENDPOINT } = naviError.getConfig(env);
    headers.set('Accept', '*/*');
...
//# sourceMappingURL=dealer-service-7d109cab.js.map