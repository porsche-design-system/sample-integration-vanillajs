...
async function getPPNDealers(env, dealerApiUrl, coordinates, country, options = {}) {
  ...
  try {
    ...
    if (env === navContentStore.Environments.LOCAL) {
      headers.set('Authorization', 'Basic ' + featureToggles.windowService.btoa(`${'undefined'}:${'undefined'}`));
    }
    const requestParams = {
      coordinates: `${coordinates.latitude},${coordinates.longitude}`,
      radius
    };
    response = await fetch(`${dealerApiUrl}/${country}/dealers?${helper.getQueryString(requestParams)}`, {
      method: 'GET',
      headers
    });
  }
  catch (err) {
    throw new navContentStore.NaviError('Unable to fetch dealers', navContentStore.NaviErrorTypes.GENERAL);
  }
  if (response.status === 400) {
    throw new navContentStore.NaviError('dealerService.getPPNDealers failed with invalid coordinates', navContentStore.NaviErrorTypes.GENERAL);
  }
  if (response.status === 404) {
    throw new navContentStore.NaviError('dealerService.getPPNDealers failed with invalid country code and therefore responded with no results', navContentStore.NaviErrorTypes.NO_RESULTS);
  }
  if (response.status !== 200) {
    throw new navContentStore.NaviError('dealerService.getPPNDealers failed with invalid response', navContentStore.NaviErrorTypes.GENERAL);
  }
  try {
    ppnDealerSearchResults = await response.json();
  }
  catch (err) {
    throw new navContentStore.NaviError('dealerService.getPPNDealers could not parse response into json format', navContentStore.NaviErrorTypes.GENERAL);
  }
...
async function getPPNDealer(env, dealerApiUrl, id, coordinates, country) {
  ...
  try {
    ...
    throw new navContentStore.NaviError('Unable to fetch a dealer', navContentStore.NaviErrorTypes.GENERAL);
  }
  if (ppnDealer === undefined) {
    throw new navContentStore.NaviError(`Requested dealer ${id} could not be found`, navContentStore.NaviErrorTypes.NO_RESULTS);
  }
  return ppnDealer;
}
...
async function getPPNDealerByRegion(id, region) {
  ...
  if (dealerResult === undefined) {
    throw new navContentStore.NaviError(`Requested dealer could not be found`, navContentStore.NaviErrorTypes.NO_RESULTS);
  }
  return dealerResult;
}
async function getPPNDealersByRegion(region) {
  ...
  if (dealersByRegionMap.has(region)) {
    ...
    const { BFF_ENDPOINT } = helper.getConfig(env);
    headers.set('Accept', '*/*');
...
exports.getAllPPNDealersForMarket = getAllPPNDealersForMarket;
exports.getPPNDealer = getPPNDealer;
exports.getPPNDealerByRegion = getPPNDealerByRegion;
exports.getPPNDealers = getPPNDealers;
exports.getPPNDealersByRegion = getPPNDealersByRegion;

//# sourceMappingURL=dealer-service-f4e71e53.js.map