...
class WindowService {
  ...
  getBreakpoint() {
    ...
    const matchingBreakpoint = Object.keys(helper.breakpoint)
      .reverse()
      .find((foundBreakpoint) => helper.hasKey(helper.breakpoint, foundBreakpoint) && clientWidth >= helper.breakpoint[foundBreakpoint]);
    return typeof matchingBreakpoint === 'string' && helper.hasKey(helper.breakpoint, matchingBreakpoint)
      ? helper.breakpoint[matchingBreakpoint]
      : 0;
  }
  get redirected() {
  ...
  async deriveChallenge(code) {
    if (code.length < 43 || code.length > 128) {
      throw new navContentStore.NaviError('invalid code length', navContentStore.NaviErrorTypes.INVALID_INPUT);
    }
  ...
  async fetchGoogleMapsApi(apiKey, url = `https://maps.googleapis.com/maps/api/js?key=${apiKey}&libraries=places`) {
    ...
    return new Promise((resolve, reject) => {
      script.onerror = () => {
        reject(new navContentStore.NaviError('Failed to load google maps script.', navContentStore.NaviErrorTypes.GENERAL));
      };
      script.onload = () => {
  ...
  async getNavigatorLatLong() {
    return new Promise((resolve, reject) => {
      ...
      }, (error) => reject(new navContentStore.NaviError(`geolocation.getCurrentPosition failed with ${error.code}: ${error.message}`, getErrorType(error))));
    });
  }
  getCookie(cookieName) {
    var _a;
    return helper.getString((_a = document.cookie
      .split('; ')
      .find((row) => row.startsWith(`${cookieName}=`))) === null || _a === void 0 ? void 0 : _a.split('=')[1]);
  }
  setCookie(cookieName, newValue) {
    document.cookie = `${cookieName}=${newValue};domain=${helper.COOKIE_DOMAIN};expires=${new Date(Date.now() + helper.MS_IN_MONTH * 10).toUTCString()};`;
  }
  btoa(data) {
    return window.btoa(data);
  }
  lockScroll() {
    this.initialBodyStyleValues = helper.shallowCopySpecifiedAttributesWithDefault(document.body.style, this.initialBodyStyleValues);
  ...
  unlockScroll() {
    Object.entries(this.initialBodyStyleValues).forEach(([key, value]) => document.body.style.setProperty(helper.camelToKebapCase(key), value));
  ...
  async waitForElement(parent, selector) {
    ...
    const el = helper.getHTMLElement(parent.querySelector(selector));
    if (el !== null) {
      return el;
    }
    return new Promise((resolve) => {
      const observer = new MutationObserver(() => {
        const newEl = helper.getHTMLElement(parent.querySelector(selector));
        if (newEl !== null) {
          resolve(newEl);
          observer.disconnect();
        }
...
function getErrorType(error) {
  switch (error.code) {
    case error.PERMISSION_DENIED: {
      return navContentStore.NaviErrorTypes.GEOLOCATION_DENIED;
    }
...
class Logger {
  ...
  log(logName, level, id, message, errorType) {
    ...
    if (env === navContentStore.Environments.TEST) {
      return;
    }
    try {
      ...
      if (env !== navContentStore.Environments.PRODUCTION) {
  ...
  error(id, err) {
    const errorType = err instanceof navContentStore.NaviError ? err.type : navContentStore.NaviErrorTypes.GENERAL;
...
class FeatureToggles {
  constructor(env = navContentStore.getEnvironment(helper.DEFAULT_ENVIRONMENT), featureEnvMap = featureJson) {
  ...
  async loadRemoteToggles() {
    try {
      const BFFEndpoint = helper.getConfig(this.env).BFF_ENDPOINT;
      ...
      throw new navContentStore.NaviError(`Load Feature Toggles failed with error: ${err}`, navContentStore.NaviErrorTypes.GENERAL);
    }
  }
  ...
  getUrlFeatureToggleOverride(feature, searchString) {
    ...
    return helper.isString(queryValue) ? this.stringToBooleanStrict(queryValue || '') : null;
  }
  getLocalStorageFeatureToggleOverride(feature) {
    const localStorageValue = localStorage.getItem(`phn.${feature}`);
    return helper.isString(localStorageValue) ? this.stringToBooleanStrict(localStorageValue) : null;
  }
...
//# sourceMappingURL=feature-toggles-56b31f71.js.map