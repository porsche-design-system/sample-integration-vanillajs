...
const doubleDrawerAnimationSequences = require('./double-drawer-animation-sequences-e0155702.js');
...
require('./animations-dcc5b59a.js');
...
const MainDrawerContent = ({ activeRoutingKey, locale, breakpoint, backButtonText, onClickBack, receivedContent }) => (index.h("div", { class: "drawer-content" },
  index.h("div", { class: "main-drawer-header" }, naviError.isString(backButtonText) ? (index.h("phn-back-button", { class: "back-button", onClick: onClickBack, text: backButtonText, analyticsId: navContentStore.navContentStore.state.menu.id, activeRoutingKey: activeRoutingKey })) : null),
...
const SideDrawerContent = ({ activeRoutingKey, locale, backButtonText, onClickBack }) => {
  return (index.h("div", { class: "drawer-content" },
    ...
    index.h("div", { class: "side-drawer-header" }, naviError.isString(backButtonText) ? (index.h("phn-back-button", { class: "back-button", onClick: onClickBack, text: backButtonText, analyticsId: navContentStore.navContentStore.state.menu.id, activeRoutingKey: activeRoutingKey })) : null),
...
const DoubleDrawer = class {
  ...
  watchRoutingKeyPointer(newPointer, oldPointer) {
    if (oldPointer < newPointer) {
      ...
      if (topLevelRoutingKey === routingKey.RoutingKeys.DEALER_SEARCH) {
        animationManager.animationManager.play('transitionSameLevelDoubleDrawer');
      }
      else {
        animationManager.animationManager.play('transitionNextLevelDoubleDrawer');
      }
    }
  }
  ...
  handleKeyDown(e) {
    ...
    const eventSourceElement = naviError.getHTMLElement(e.target);
    if (eventSourceElement === null) {
      return;
    }
    const focusableSideDrawerElements = helper.getAllFocusableElements(this.el.querySelector('#side-drawer'));
    let focusableMainDrawerElements = helper.getAllFocusableElements(this.el.querySelector('#main-drawer'));
    ...
    if (!e.shiftKey) {
      if (eventSourceElement === highlightedSideDrawerElement) {
        e.preventDefault();
        naviError.focusOnElement(focusableMainDrawerElements[0]);
        return;
      }
      if (eventSourceElement === lastSideDrawerElement) {
        e.preventDefault();
        if (closeButton !== null) {
          naviError.focusOnElement(closeButton);
        }
      }
      if (eventSourceElement === lastMainDrawerElement) {
        ...
        const nextSideDrawerElement = naviError.getHTMLElement(focusableSideDrawerElements[nextSideDrawerElementIndex]);
        naviError.focusOnElement(nextSideDrawerElement);
      }
      if (eventSourceElement === closeButton) {
        e.preventDefault();
        naviError.focusOnElement(focusableSideDrawerElements[0]);
      }
    }
    else if (e.shiftKey) {
      if (eventSourceElement === focusableMainDrawerElements[0]) {
        ...
        if (highlightedSideDrawerElement !== null) {
          naviError.focusOnElement(highlightedSideDrawerElement);
        }
      }
      if (eventSourceElement === closeButton) {
        e.preventDefault();
        naviError.focusOnElement(lastSideDrawerElement);
      }
    }
  }
  ...
  focusOnInitialElement() {
    ...
    const autoFocusElement = naviError.getHTMLElement(this.el.querySelector('.autofocus'));
    ...
    if (this.usingKeyboard) {
      ...
      if (naviError.isString(focusedRoutingKey)) {
        focusableElement = naviError.getHTMLElement(this.el.querySelector(`[data-id="${focusedRoutingKey}"]`));
      }
      if (focusableElement === null) {
        focusableElement = naviError.getHTMLElement(this.el.querySelector(`${naviError.FOCUSABLE_ELEMENTS_SELECTOR}`));
      }
      if (focusableElement !== null) {
        naviError.focusOnElement(focusableElement);
      }
      return;
    }
  ...
  async goBackToPreviousMenu() {
    if (this.routingKeyPointer === this.animatingRoutingKeyPointer) {
      ...
      const topLevelRoutingKey = routingKey.findTopLevelRoutingKey(this.routingKeyHistory[this.routingKeyPointer]);
      if (topLevelRoutingKey === routingKey.RoutingKeys.DEALER_SEARCH) {
        animationManager.animationManager.play('transitionSameLevelDoubleDrawerReverse');
      }
      else {
        animationManager.animationManager.play('transitionPreviousLevelDoubleDrawer');
      }
    }
  }
  async updateRouting() {
  ...
  render() {
    ...
    return (index.h(index.Host, { role: "dialog", "aria-modal": "true", "aria-label": "navigation modal dialog", style: { left: '-1000px' } }, index.h("phn-p-button", { variant: "secondary", class: "close-button", theme: "dark", "hide-label": "true", icon: 'close', aria: "{ 'aria-label': 'close modal dialog' }", "data-test-id": "close-button", onClick: () => {
        this.sendDataEventOfCloseButtonClick();
        this.clickDrawerHeaderClose.emit();
      } }), naviError.isString(this.animatingRoutingKey) ? (index.h(index.Fragment, null, index.h("div", { class: "drawer", id: "side-drawer" }, index.h(SideDrawerContent, { activeRoutingKey: this.animatingRoutingKey, locale: this.locale, backButtonText: wasPreviouslyOnDealerSearch ? '' : backButtonText, onClickBack: () => this.goBackToPreviousMenu() })), index.h("div", { class: "drawer", id: "main-drawer" }, index.h(MainDrawerContent, { activeRoutingKey: this.animatingRoutingKey, breakpoint: this.breakpoint, locale: this.locale, backButtonText: isOnDealerDetails ? backButtonText : '', receivedContent: this.receivedContent, onClickBack: () => this.goBackToPreviousMenu() }), index.h("div", { class: "bottom-gradient" })))) : null));
  }
