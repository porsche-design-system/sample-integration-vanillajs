...
const BurgerButton = class {
  ...
  render() {
    ...
    return (index.h(index.Host, { class: `${naviError.constructThemeClass(this.mode, this.theme)}-theme` }, index.h("phn-p-button-pure", { theme: naviError.getRightTheme(this.theme, this.mode), onClick: () => this.sendDataLayerEvent(menu), aria: { 'aria-haspopup': 'dialog', 'aria-label': 'open the menu' }, "align-label": this.alignLabel, icon: "menu-lines", loading: this.isLoading, size: "inherit", hideLabel: this.hideLabel }, menu.text)));
  }
};
...
const MyPorscheIconButton = class {
  ...
  render() {
    ...
    return (index.h(index.Host, null, index.h("phn-p-button-pure", { icon: `user${isLoggedIn ? '-filled' : ''}`, "hide-label": "true", stretch: "true", "data-id": `${routingKey.RoutingKeys.MY_PORSCHE}-contextual-button`, aria: "{ 'aria-haspopup': 'dialog', 'aria-label' : 'open the my porsche menu' }", "data-test-id": "myporsche-button", theme: naviError.getRightTheme(this.theme, this.mode), onMouseEnter: () => {
        this.isMyPorscheHoveredOn = true;
      }, onMouseLeave: () => {
        this.isMyPorscheHoveredOn = false;
      }, onClick: () => {
        this.sendDataLayerEvent();
      } }), unreadMessagesCount > 0 ? (index.h("span", { class: `myporsche-unread-messages-indicator ${this.mode === 'hero' ? this.theme : naviError.NavigationTheme.light}` })) : null));
  }
};
