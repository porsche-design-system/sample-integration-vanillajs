...
const BurgerButton = class {
  ...
  sendDataLayerEvent(menu) {
    globalDataLayerService.globalDataLayerService.pushDataLayerEvent(navContentStore.EventActions.MORE_CLICK, {
      componentClick: {
  ...
  render() {
    ...
    return (index.h(index.Host, { class: `${helper.constructThemeClass(this.mode, this.theme)}-theme` }, index.h("phn-p-button-pure", { theme: helper.getRightTheme(this.theme, this.mode), onClick: () => this.sendDataLayerEvent(menu), aria: { 'aria-haspopup': 'dialog', 'aria-label': 'open the menu' }, "align-label": this.alignLabel, icon: "menu-lines", loading: this.isLoading, size: "inherit", hideLabel: this.hideLabel }, menu.text)));
  }
};
...
const MyPorscheIconButton = class {
  constructor(hostRef) {
    index.registerInstance(this, hostRef);
    this.mode = helper.NavigationMode.navbar;
    this.theme = helper.NavigationTheme.light;
  ...
  sendDataLayerEvent() {
    globalDataLayerService.globalDataLayerService.pushDataLayerEvent(navContentStore.EventActions.CONTEXTUAL_BUTTON_CLICK, {
      componentClick: {
        clickElementId: routingKey.RoutingKeys.MY_PORSCHE,
  ...
  render() {
    const { unreadMessagesCount, loggedInState } = navContentStore.navStateStore.state;
    const isLoggedIn = helper.IsLoggedIn(loggedInState);
    return (index.h(index.Host, null, index.h("phn-p-button-pure", { icon: `user${isLoggedIn ? '-filled' : ''}`, "hide-label": "true", stretch: "true", "data-id": `${routingKey.RoutingKeys.MY_PORSCHE}-contextual-button`, aria: "{ 'aria-haspopup': 'dialog', 'aria-label' : 'open the my porsche menu' }", "data-test-id": "myporsche-button", theme: helper.getRightTheme(this.theme, this.mode), onMouseEnter: () => {
        this.isMyPorscheHoveredOn = true;
      }, onMouseLeave: () => {
        this.isMyPorscheHoveredOn = false;
      }, onClick: () => {
        this.sendDataLayerEvent();
      } }), unreadMessagesCount > 0 ? (index.h("span", { class: `myporsche-unread-messages-indicator ${this.mode === 'hero' ? this.theme : helper.NavigationTheme.light}` })) : null));
  }
};
MyPorscheIconButton.style = myPorscheIconButtonCss;

exports.phn_burger_button = BurgerButton;
exports.phn_my_porsche_icon_button = MyPorscheIconButton;

//# sourceMappingURL=phn-burger-button_2.cjs.entry.js.map