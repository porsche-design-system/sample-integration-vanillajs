...
const DealerDetailsDrawer = class {
  ...
  componentDidLoad() {
    ...
    globalDataLayerService.globalDataLayerService.pushDataLayerEvent(navContentStore.EventActions.DEALER_DETAILS_SCREEN_LOAD, {
      ...
      componentSearch: {
        searchTerm: dealerSearchResultsStore.dealerSearchResultsStore.state.searchTerm
      },
      partner: {
  ...
  getDealer() {
    const { ppnDealer: dealer } = dealerSearchResultsStore.dealerSearchResultsStore.state.ppnDealers[this.dealerId] || {};
    return dealer;
  }
  renderHeading(name) {
...
const DealerSearchByNumberOfDealers = class {
  ...
  async fetchAllDealersForMarket() {
    ...
    try {
      ...
      dealerSearchResultsStore.dealerSearchResultsStore.state.results = ppnDealerSearchResults.map(({ ppnDealer: dealer }) => dealer.id);
      dealerSearchResultsStore.dealerSearchResultsStore.state.ppnDealers = ppnDealerSearchResults.reduce((dealers, dealerSearchResult) => (Object.assign(Object.assign({}, dealers), { [dealerSearchResult.ppnDealer.id]: dealerSearchResult })), dealerSearchResultsStore.dealerSearchResultsStore.state.ppnDealers);
    }
    catch (err) {
  ...
  reset() {
    this.formErrorMessage = null;
    this.isLoading = false;
    dealerSearchResultsStore.dealerSearchResultsStore.state.results = null;
  }
  render() {
    if (this.isLoading) {
...
const DealerSearchFormDropdown = class {
  ...
  onCitySelect(event) {
    ...
    dealerSearchResultsStore.dealerSearchResultsStore.state.ppnDealers =
    ...
    dealerSearchResultsStore.dealerSearchResultsStore.state.results = Object.keys(dealerSearchResultsStore.dealerSearchResultsStore.state.ppnDealers);
    dealerSearchResultsStore.dealerSearchResultsStore.state.searchTerm = `${this.selectedProvince}, ${this.selectedCity}`;
  }
  ...
  emitPageLoadEvent() {
    const { results, searchTerm } = dealerSearchResultsStore.dealerSearchResultsStore.state;
    const { dealer } = navContentStore.navContentStore.state;
    globalDataLayerService.globalDataLayerService.pushDataLayerEvent(navContentStore.EventActions.DEALER_SEARCH_SCREEN_LOAD, {
      componentDisplay: {
        displayElementId: analyticsManager.analyticsManager.calculateDisplayElementId(),
  ...
  render() {
    const { results } = dealerSearchResultsStore.dealerSearchResultsStore.state;
    const { dealer: dealerContent } = navContentStore.navContentStore.state;
    const searchText = dealerSearchResultsStore.dealerSearchResultsStore.state.searchTerm;
    if (this.isLoading) {
      return (index.h("div", { class: "spinner" }, index.h("phn-p-spinner", { size: "medium", aria: { 'aria-label': 'Loading page content' } })));
    }
...
const DealerSearchFormGeolocation = class {
  constructor(hostRef) {
    ...
    this.formErrorMessage = null;
    this.isLoading = false;
    this.showGeolocationButton = dealerSearchResultsStore.dealerSearchResultsStore.state.results === null;
    this.animateResults = false;
    this.searchTerm = dealerSearchResultsStore.dealerSearchResultsStore.state.searchTerm;
    this.showSkeleton = true;
  }
  ...
  disconnectedCallback() {
    dealerSearchResultsStore.dealerSearchResultsStore.state.searchTerm = this.searchTerm;
  ...
  async fetchDealers(getSearchTerms) {
    ...
    try {
      ...
      dealerSearchResultsStore.dealerSearchResultsStore.state.searchTerm = searchTerm;
      dealerSearchResultsStore.dealerSearchResultsStore.state.results = ppnDealerSearchResults.map(({ ppnDealer: dealer }) => dealer.id);
      dealerSearchResultsStore.dealerSearchResultsStore.state.ppnDealers = ppnDealerSearchResults.reduce((dealers, dealerSearchResult) => (Object.assign(Object.assign({}, dealers), { [dealerSearchResult.ppnDealer.id]: dealerSearchResult })), dealerSearchResultsStore.dealerSearchResultsStore.state.ppnDealers);
  ...
  render() {
    const { dealer: dealerContent } = navContentStore.navContentStore.state;
    const { results } = dealerSearchResultsStore.dealerSearchResultsStore.state;
    return (index.h(index.Host, { class: this.showSkeleton ? 'skeleton' : '' }, index.h("div", { class: "input-container" }, this.showSkeleton ? index.h("div", { class: "skeleton-autocomplete-search-field" }) : null, index.h("phn-autocomplete-search-field", { placeholder: dealerContent.search.form.inputPlaceholder, initialValue: dealerSearchResultsStore.dealerSearchResultsStore.state.searchTerm, predictions: this.predictions, onSearchInput: (e) => this.debounceTimeout.set(() => this.onDealerSearchEntry(e.detail)), onClearSearch: () => {
        this.predictions = [];
        dealerSearchResultsStore.dealerSearchResultsStore.state.searchTerm = '';
        this.searchTerm = '';
        this.reset();
        this.clearSearchEvent.emit();
        this.showGeolocationButton = true;
...
function DealerSearchFormItemCard({ dealer, showOverlay, locale }) {
  ...
  const onDealerCardClick = (ppnDealer) => {
    ...
    routingManager.addRoute([ppnDealer.id]);
    const { dealer: dealerContent } = navContentStore.navContentStore.state;
    const { results, searchTerm } = dealerSearchResultsStore.dealerSearchResultsStore.state;
...
const DealerSearchResults = class {
  ...
  render() {
    const { results, ppnDealers } = dealerSearchResultsStore.dealerSearchResultsStore.state;
    const { error: errorContent } = navContentStore.navContentStore.state;
    if (results === null)
      return null;
    ...
    return (index.h(index.Host, null, index.h(index.Fragment, null, results.map((dealerId) => {
      ...
      return index.h(DealerSearchFormItemCard, { dealer: dealer, showOverlay: this.showOverlay, locale: this.locale });
