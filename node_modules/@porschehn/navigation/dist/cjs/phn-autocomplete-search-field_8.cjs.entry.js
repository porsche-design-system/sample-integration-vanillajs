...
const dealerService = require('./dealer-service-7d109cab.js');
const windowService = require('./window-service-c9a19f26.js');
const dataLayerEventSender = require('./data-layer-event-sender-297098a6.js');
...
const AutocompleteSearchField = class {
  ...
  render() {
    return (index.h("form", { class: "form-container", onSubmit: (event) => {
        ...
        this.onSubmit();
      } }, index.h("phn-p-text-field-wrapper", { "data-test-id": "dealer-search-form-input", state: naviError.isString(this.errorMessage) ? 'error' : 'none', message: this.errorMessage }, index.h("input", { class: "autofocus", "aria-label": "Find a dealer by typing a city name or zip code", type: "search", placeholder: this.placeholder, autocomplete: "off", role: "combobox", "data-validate": "required", "aria-expanded": (this.predictions.length > 0 && this.showPredictions).toString(), "aria-controls": "predictions", "aria-autocomplete": "list", "aria-activedescendant": this.showPredictions ? String(this.highlightedPredictionIndex) : undefined, value: this.searchterm, onInput: (event) => {
        var _a;
        event.preventDefault();
        this.showPredictions = true;
        this.highlightedPredictionIndex = -1;
        const value = (_a = naviError.getHTMLInputElement(event.target)) === null || _a === void 0 ? void 0 : _a.value;
        if (!naviError.isString(value)) {
          this.searchterm = '';
          this.showPredictions = false;
      ...
      } }, index.h("phn-p-text", { color: "inherit" }, prediction.text))))), index.h("img", { class: "powered-by-google", src: index.getAssetPath(`${naviError.getConfig(navContentStore.navStateStore.state.env).ASSETS_URL}/icon-assets/powered_by_google_on_white.png`), alt: "powered by Google" }))) : null));
  }
...
const DealerDetailsDrawer = class {
  ...
  render() {
    ...
    const name = language === 'en' ? ppnDealer.name : naviError.isString(ppnDealer.nameLocalized) ? ppnDealer.nameLocalized : ppnDealer.name;
    // postalCode is currently not available in addressLocalized
    const hasAddress = naviError.isString(address.street) || naviError.isString(ppnDealer.address.postalCode) || naviError.isString(address.city);
    const hasPhoneNumber = naviError.isString(ppnDealer.contactDetails.phoneNumber);
    const hasEmail = naviError.isString(ppnDealer.contactDetails.emailAddress);
    const hasHomepage = naviError.isString(ppnDealer.contactDetails.homepage);
    const hasTestdrive = testDrive && naviError.isString(testDrive === null || testDrive === void 0 ? void 0 : testDrive.text) && naviError.isString(testDrive === null || testDrive === void 0 ? void 0 : testDrive.link);
    const hasName = naviError.isString(ppnDealer.nameLocalized) || naviError.isString(ppnDealer.name);
...
const DealerSearchByNumberOfDealers = class {
  ...
  async fetchAllDealersForMarket() {
    ...
    try {
      ...
      const ppnDealerSearchResults = await dealerService.getAllPPNDealersForMarket(env, naviError.getConfig(env).DEALER_API_URL_ALL_DEALERS, country);
      ...
      this.formErrorMessage = naviError.getErrorMessage(errorContent, err);
    }
    finally {
      this.isLoading = false;
  ...
  emitPageLoadEvent() {
    ...
    globalDataLayerService.globalDataLayerService.pushDataLayerEvent(navContentStore.EventActions.DEALER_SEARCH_SCREEN_LOAD, {
      ...
      componentSearch: {
        searchTerm: naviError.DEALER_MARKET_SEARCH_TERM,
  ...
  render() {
    ...
    if (naviError.isString(this.formErrorMessage))
...
const DealerSearchFormDropdown = class {
  ...
  async getDealersByRegion() {
    ...
    try {
      ...
      featureToggles.logger.error('FETCH_DEALERS', err);
      this.errorMessage = naviError.getErrorMessage(errorContent, err);
    }
    finally {
      this.isLoading = false;
  ...
  emitPageLoadEvent() {
    ...
    globalDataLayerService.globalDataLayerService.pushDataLayerEvent(navContentStore.EventActions.DEALER_SEARCH_SCREEN_LOAD, {
      ...
      componentSearch: {
        searchTerm: naviError.isString(searchTerm) ? searchTerm : naviError.GEOLOCATION_BUTTON_SEARCH_TERM,
  ...
  render() {
    ...
    return (index.h(index.Host, null, results && results.length > 0 ? (index.h("form", { class: "form-container", onSubmit: (event) => {
    ...
    }))), index.h("phn-p-select-wrapper", { style: { display: this.showSkeleton ? 'none' : '' }, class: "city-select-form-field", filter: "true", label: dealerContent.search.dropdown.city, required: true }, index.h("select", { disabled: !naviError.isString(this.selectedProvince), style: { display: this.showSkeleton ? 'none' : '' }, name: "dealers-cities", onChange: (event) => {
      ...
      ? city.cityName
      : city.cityNameLocalized))))), naviError.isString(this.errorMessage) ? (index.h(ErrorMessage, { errorMessage: this.errorMessage, onDismiss: () => this.reset() })) : null))));
  }
  get el() { return index.getElement(this); }
};
...
class MapsService {
  ...
  async init(apiKey) {
    try {
      const google = await windowService.windowService.fetchGoogleMapsApi(apiKey);
      this.autocompleteService = new google.maps.places.AutocompleteService();
      this.placesService = new google.maps.places.PlacesService(new google.maps.Map(windowService.windowService.createDiv()));
      this.geocoderService = new google.maps.Geocoder();
      this.getLocation = (latLong) => new google.maps.LatLng(latLong.latitude, latLong.longitude);
    }
    catch (err) {
      throw new naviError.NaviError(`windowService.fetchGoogleMapsApi failed with error: ${err}`, naviError.NaviErrorTypes.GENERAL);
    }
  }
  async getPredictions(input, locale) {
    ...
    const predictions = await new Promise((resolve, reject) => {
      if (this.autocompleteService === null) {
        reject(new naviError.NaviError('autocompleteService is not initialised', naviError.NaviErrorTypes.GENERAL));
        return;
      }
      this.autocompleteService.getPlacePredictions({
        ...
        ? resolve(predictions)
        : reject(new naviError.NaviError(`autocompleteService.getPlacePredictions failed with ${status}`, getErrorType(status))));
    });
    return ((_a = predictions === null || predictions === void 0 ? void 0 : predictions.filter(({ place_id, description }) => naviError.isString(place_id) && naviError.isString(description)).map(({ place_id: id, description: text }) => ({ id, text }))) !== null && _a !== void 0 ? _a : []);
  }
  async getPlaceCoordinates(placeId) {
    var _a, _b, _c, _d;
    const response = await new Promise((resolve, reject) => {
      if (this.placesService === null) {
        reject(new naviError.NaviError('placesService is not initialised', naviError.NaviErrorTypes.GENERAL));
        return;
      }
      this.placesService.getDetails({
        ...
        ? resolve(response)
        : reject(new naviError.NaviError(`placeService.getDetails failed with ${status}`, getErrorType(status))));
    });
    ...
    if (typeof latitude !== 'number' || typeof longitude !== 'number') {
      throw new naviError.NaviError(`placesService.getDetails did not find latitude or longitude for placeId: ${placeId}`, naviError.NaviErrorTypes.NO_RESULTS);
    }
    return { latitude, longitude };
  }
  async getFormattedAddressFromCoordinates(latLong) {
    if (this.getLocation === null) {
      throw new naviError.NaviError('getLocation is not initialised', naviError.NaviErrorTypes.GENERAL);
    }
    const location = this.getLocation(latLong);
    const response = await new Promise((resolve, reject) => {
      if (this.geocoderService === null) {
        reject(new naviError.NaviError('geocoderService is not initialised', naviError.NaviErrorTypes.GENERAL));
        return;
      }
      this.geocoderService.geocode({ location }, (response, status) => {
        status === 'OK'
          ? resolve(response)
          : reject(new naviError.NaviError(`geocoderService.geocode failed with status ${status}`, getErrorType(status)));
      });
    });
    if (response === null || response.length === 0) {
      throw new naviError.NaviError('geocoderService.geocode did not find the location based on coordinates.', naviError.NaviErrorTypes.NO_RESULTS);
    }
...
function getErrorType(status) {
  switch (status) {
    case 'INVALID_REQUEST': {
      return naviError.NaviErrorTypes.INVALID_INPUT;
    }
    case 'NOT_FOUND':
    case 'ZERO_RESULTS': {
      return naviError.NaviErrorTypes.NO_RESULTS;
    }
...
const DealerSearchFormGeolocation = class {
  ...
  emitResultsLoadEvent(searchTerm) {
    const { results } = abTestingConstructorService.dealerSearchResultsStore.state;
    const { dealer: dealerContent } = navContentStore.navContentStore.state;
    const componentSearchSearchTerm = naviError.isString(searchTerm) ? searchTerm : naviError.GEOLOCATION_BUTTON_SEARCH_TERM;
    const numberOfResults = (results === null || results === void 0 ? void 0 : results.length) || 0;
    globalDataLayerService.globalDataLayerService.pushDataLayerEvent(navContentStore.EventActions.DEALER_RESULTS_SCREEN_LOAD, {
      componentDisplay: {
        displayElementId: analyticsManager.analyticsManager.getDealerResultsLoadDisplayElementId(navContentStore.DEALER_SEARCH_RESULT_DISPLAY_ELEMENT_ID_DOUBLE_DRAWER),
  ...
  async fetchDealers(getSearchTerms) {
    ...
    try {
      ...
      const { env } = navContentStore.navStateStore.state;
      const ppnDealerSearchResults = await dealerService.getPPNDealers(env, naviError.getConfig(env).DEALER_API_URL, placeCoordinates, dealerSearchHelper.getRegionSpecificCountry(navContentStore.splitLocale(this.locale).country));
  ...
  async onSubmit(prediction) {
    ...
    if (!naviError.isString(this.searchTerm)) {
    ...
    await this.fetchDealers(async () => {
      if (prediction === null) {
        throw new naviError.NaviError('Dealer search form submitted without prediction', naviError.NaviErrorTypes.INVALID_INPUT);
      }
      this.searchTerm = prediction.text;
  ...
  async getCurrentLocation() {
    const { dealer: dealerContent } = navContentStore.navContentStore.state;
    await this.fetchDealers(async () => {
      const placeCoordinates = await windowService.windowService.getNavigatorLatLong();
      ...
      globalDataLayerService.globalDataLayerService.pushDataLayerEvent(navContentStore.EventActions.DEALER_GEOLOCATION_CLICK, {
        ...
        componentSearch: {
          searchTerm: naviError.GEOLOCATION_BUTTON_SEARCH_TERM
        }
      });
      return [formattedAddress, placeCoordinates];
    });
    this.emitResultsLoadEvent(naviError.GEOLOCATION_BUTTON_SEARCH_TERM);
  }
  async onDealerSearchEntry(searchTerm) {
    ...
    try {
      ...
      this.errorMessage = naviError.getErrorMessage(error, err);
      this.predictions = [];
    }
  }
  reset() {
  ...
  render() {
    ...
    return (index.h(index.Host, { class: this.showSkeleton ? 'skeleton' : '' }, index.h("div", { class: "input-container" }, this.showSkeleton ? index.h("div", { class: "skeleton-autocomplete-search-field" }) : null, index.h("phn-autocomplete-search-field", { placeholder: dealerContent.search.form.inputPlaceholder, initialValue: abTestingConstructorService.dealerSearchResultsStore.state.searchTerm, predictions: this.predictions, onSearchInput: (e) => this.debounceTimeout.set(() => this.onDealerSearchEntry(e.detail)), onClearSearch: () => {
        ...
        this.clearSearchEvent.emit();
        this.showGeolocationButton = true;
      }, onSubmitSearch: (e) => this.onSubmit(e.detail), errorMessage: naviError.getString(this.errorMessage), style: { display: this.showSkeleton ? 'none' : '' } })), this.isLoading && (index.h("div", { class: "spinner" }, index.h("phn-p-spinner", { size: "medium", aria: { 'aria-label': 'Loading page content' } }))), this.showGeolocationButton && results === null ? (index.h(index.Fragment, null, index.h("div", { class: `or-container ${this.isLoading ? 'animate-out' : ''}`, onAnimationEnd: () => {
        this.showGeolocationButton = false;
      } }, this.showSkeleton ? index.h("div", { class: "skeleton-or-text" }) : null, index.h("phn-p-text", { color: "inherit", size: "inherit", tag: "div", "data-test-id": "dealer-search-or-label", style: { display: this.showSkeleton ? 'none' : '' } }, dealerContent.search.or), this.showSkeleton ? index.h("div", { class: "skeleton-geolocation-button" }) : null, index.h("phn-p-button", { class: 'geolocation', onClick: () => this.getCurrentLocation(), variant: "tertiary", icon: "locate", style: { display: this.showSkeleton ? 'none' : '' } }, dealerContent.search.geolocation.text)))) : null, naviError.isString(this.formErrorMessage) ? (index.h(ErrorMessage, { errorMessage: this.formErrorMessage, onDismiss: () => this.reset() })) : null));
  }
  get el() { return index.getElement(this); }
};
...
const MenuItemCard = class {
  ...
  handleKeyDown(e) {
    var _a;
    if (e.key === 'Enter') {
      const anchorElement = (_a = naviError.getHTMLElement(e.target)) === null || _a === void 0 ? void 0 : _a.querySelector('a');
      const targetElement = anchorElement ? anchorElement : naviError.getHTMLElement(e.target);
      if (targetElement) {
        targetElement.click();
  ...
  renderTitle() {
    return (index.h("div", { class: "phn-flex-row phn-align-center phn-justify-between title-container" }, index.h("phn-p-heading", { tag: "h3", size: "medium" }, this.text), naviError.isString(this.iconLeft) ? index.h("phn-p-icon", { name: this.iconLeft }) : null));
  }
  renderSubtitle() {
    if (!naviError.isString(this.subtext)) {
      return null;
    }
  ...
  render() {
    ...
    if (naviError.isString(this.link))
...
const MyPorscheLogin = class {
  ...
  render() {
    return (index.h(index.Fragment, null, index.h("div", { class: "button-container" }, index.h("phn-p-button", { variant: "primary", class: "login", onClick: () => {
      ...
      } }, this.login.text), this.more && naviError.isString(this.more.link) ? (index.h("phn-p-link", { variant: "tertiary", class: "more", href: this.more.link, onClick: () => {
