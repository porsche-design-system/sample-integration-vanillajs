...
function splitLocale(localeStr) {
  if (!naviError.isString(localeStr)) {
    throw new naviError.NaviError('empty locale passed', naviError.NaviErrorTypes.NO_INPUT);
  }
  ...
  if (!naviError.isString(language) || !naviError.isString(country)) {
    throw new naviError.NaviError(`invalid locale ${localeStr} passed`, naviError.NaviErrorTypes.INVALID_INPUT);
  }
  return {
    language,
    country: naviError.hasKey(RegionLocaleMap, country) ? RegionLocaleMap[country] : country
  };
}
...
const navStateStore = createStore({
  loggedInState: naviError.LOGGED_OUT_USER_STATUS,
  unreadMessagesCount: naviError.DEFAULT_UNREAD_MESSAGES_COUNT,
  userConsent: false,
  env: getEnvironment(naviError.DEFAULT_ENVIRONMENT),
  app: naviError.Application.default
});
...
//# sourceMappingURL=nav-content-store-792bb73e.js.map